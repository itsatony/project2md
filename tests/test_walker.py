...
def config():
    config = Config()
    config.include = PathPatterns(files=["*.py", "*.md"], dirs=["src/", "docs/"])
    config.exclude = PathPatterns(files=["*.pyc", "test_*"], dirs=["__pycache__/", "tests/"])
    return config
...
def test_error_handling(walker, tmp_path, caplog):
    ...
    with pytest.raises(WalkerError, match=expected_error):
        try:
            ...
            print(f"Caught exception: {type(e).__name__}: {e!s}")
