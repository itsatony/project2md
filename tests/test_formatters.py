...
from project2md.formatters.base import FormatterError
...
class TestJSONFormatter:
    ...
    def test_json_special_content_handling(self, config, tmp_path):
        ...
        files = [(tmp_path / "test.json", '{"nested": {"json": true, "array": [1,2,3]}}')]
...
def test_formatter_error_handling():
    ...
    with pytest.raises(FormatterError):
        json_formatter.generate_output(
            Path("/nonexistent"), [], {}, Path("/nonexistent/output.json")
        )
    ...
    with pytest.raises(FormatterError):
        yaml_formatter.generate_output(
            Path("/nonexistent"), [], {}, Path("/nonexistent/output.yaml")
        )
