...
class GitHandler:
    ...
    def prepare_repository(self, force: bool = False) -> Path:
        ...
        try:
            ...
            raise GitError(f"Failed to prepare repository: {e!s}") from e
    ...
    def _clone_repository(self) -> Path:
        ...
        try:
            ...
            if not self.config.repo_url:
                raise GitError("No repository URL configured")
            ...
            if "Remote branch not found" in str(e):
                raise GitError(f"Branch '{self.config.branch}' not found in repository") from e
            raise GitError(f"Git clone failed: {e!s}") from e
        except Exception as e:
            self._cleanup_temp_dir()
            raise GitError(f"Failed to clone repository: {e!s}") from e
    ...
    def _validate_local_repository(self, force: bool = False) -> Path:
        ...
        try:
            ...
            try:
                ...
                if self.config.branch != "main" and self.config.branch != self.get_current_branch():
                    try:
                        ...
                        raise GitError(
                            f"Failed to switch to branch '{self.config.branch}': {e!s}"
                        ) from e
            ...
            if isinstance(e, GitError):
                raise
            raise GitError(f"Failed to validate local repository: {e!s}") from e
    ...
    def get_repo_info(self) -> dict:
        ...
        if not self._repo:
            return {"branch": "unknown", "is_git_repo": False}
    ...
    def _progress_printer(
        ...
        if max_count:
            ...
            self.progress.console.print(f"Clone progress: {percentage:.1f}% ({message})", end="\r")
