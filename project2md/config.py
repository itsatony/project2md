...
class OutputFormat(Enum):
    ...
    def from_string(cls, value: str) -> "OutputFormat":
        try:
            ...
            raise ConfigError(f"Invalid output format: {value}") from None
...
class Config:
    ...
    # default_factory, not a call: a dataclass default is evaluated once at
    # import time, so Path.cwd() would freeze the interpreter's startup cwd.
    ...
    def from_dict(cls, data: dict) -> "Config":
        ...
        try:
            ...
            include = PathPatterns(
                files=include_data.get("files", []), dirs=include_data.get("dirs", [])
            )

            exclude = PathPatterns(
                files=exclude_data.get("files", []), dirs=exclude_data.get("dirs", [])
            )

            return cls(general=general, output=output, include=include, exclude=exclude)
        except Exception as e:
            raise ConfigError(f"Error parsing config data: {e}") from e
    ...
    def create_default_config(cls, path: Path) -> None:
        ...
        config_dict = {
            ...
            "output": {"format": "markdown", "stats": True},
    ...
    def _validate_patterns(patterns: list[str], context: str) -> None:
        ...
        for pattern in patterns:
            ...
            if "\\" in pattern and not any(c in pattern for c in "*?[]{}"):
                raise ConfigError(
                    f"Invalid pattern in {context}: {pattern} (Invalid escape character)"
                )
            ...
            try:
                ...
                raise ConfigError(f"Invalid pattern in {context}: {pattern} ({e})") from e
    ...
    def save(self, path: str | Path) -> None:
        ...
        config_dict = {
            ...
            "output": {"format": self.output.format.value, "stats": self.output.stats},
            "include": {"files": self.include.files, "dirs": self.include.dirs},
            "exclude": {"files": self.exclude.files, "dirs": self.exclude.dirs},
        }
        ...
        try:
            ...
            raise ConfigError(f"Error saving config to {path}: {e}") from e
