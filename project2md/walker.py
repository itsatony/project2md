...
class FileSystemWalker:
    ...
    def collect_files(self, root_path: Path) -> list[Path]:
        ...
        try:
            ...
            # Debug log patterns
            logger.debug(
                f"Include patterns: {self.config.include.files + self.config.include.dirs}"
            )
            logger.debug(
                f"Exclude patterns: {self.config.exclude.files + self.config.exclude.dirs}"
            )
            ...
            raise WalkerError(f"Error collecting files: {e!s}") from e
        except NotADirectoryError as e:
            raise WalkerError(f"Error collecting files: {e!s}") from e
        except Exception as e:
            raise WalkerError(f"Error collecting files: Unexpected error - {e!s}") from e
    ...
    def _setup_patterns(self, root_path: Path) -> None:
        ...
        if self.config.include.files or self.config.include.dirs:
            self._include_spec = pathspec.PathSpec.from_lines(
                "gitwildmatch", self.config.include.files + self.config.include.dirs
            )

        if self.config.exclude.files or self.config.exclude.dirs:
            self._exclude_spec = pathspec.PathSpec.from_lines(
                "gitwildmatch", self.config.exclude.files + self.config.exclude.dirs
            )
    ...
    def _should_read_file(self, path: Path) -> bool:
        ...
        try:
            return (
                path.exists()
                and path.is_file()
                and path.suffix.lower() not in self.BINARY_EXTENSIONS
